...
// ── Donut chart ───────────────────────────────────────
const VB_SIZE = 200;
const PIE_CX = 100, PIE_CY = 100, PIE_R = 80, PIE_IR = 42;
...
function DonutChart({ tasks, totalWeight, onBoundaryDrag, onHover }) {
  ...
  tasks.forEach((task, i) => {
    ...
    slices.push({ id: task.id, d, color: PALETTE[task.colorIdx], ratio, name: task.name || '항목', locked: task.locked });
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${VB_SIZE} ${VB_SIZE}`} className="pie-svg">
      {slices.map(s => (
        <path
      ...
      {hs ? (
        <>
          <text x={PIE_CX} y={PIE_CY - 6} textAnchor="middle" fontSize="18" fontWeight="800" fill="#1a1d23">{hs.ratio.toFixed(2)}</text>
          <text x={PIE_CX} y={PIE_CY + 12} textAnchor="middle" fontSize="9" fill="#7a8494">{trunc(hs.name, 7)}</text>
        </>
      ) : (
        <>
          <text x={PIE_CX} y={PIE_CY - 6} textAnchor="middle" fontSize="20" fontWeight="800" fill="#1a1d23">1.00</text>
          <text x={PIE_CX} y={PIE_CY + 12} textAnchor="middle" fontSize="10" fill="#7a8494">MM</text>
        </>
      )}
      {tasks.length > 1 && handles.map(h => h.isLocked ? (
        <g key={`h-${h.id0}`} style={{ pointerEvents: 'none' }}>
          <circle cx={f(h.hx)} cy={f(h.hy)} r="5" fill="rgba(255,255,255,.75)" stroke="#d0d5e0" strokeWidth="1.5" />
          <text x={f(h.hx)} y={f(h.hy)} textAnchor="middle" dominantBaseline="middle" fontSize="5">🔒</text>
        </g>
      ) : (
        <circle
          key={`h-${h.id0}`}
          ...
          style={{ cursor: 'grab', filter: 'drop-shadow(0 1px 2px rgba(0,0,0,.25))' }}
...
export default function MMPlanner() {
  ...
  function startEditMm(id, ratio, mode = 'mm') {
    ...
    const hours = days * 8;
    const initVal = mode === 'mm'   ? (ratio > 0 ? ratio.toFixed(2) : '')
  ...
  function commitEditMm(id) {
    if (!editMm) return;
    const raw = parseFloat(editMm.val);
    let v = NaN;
    if (!isNaN(raw) && raw >= 0) {
      if (editMm.mode === 'mm')   v = raw;
      else if (editMm.mode === 'day'  && workingDays > 0) v = raw / workingDays;
  ...
  const handlePieDrag = useCallback((e, id0, id1, svgRef) => {
    ...
    const scaleX  = VB_SIZE / svgRect.width;
    const scaleY  = VB_SIZE / svgRect.height;
  ...
  return (
    <>
      ...
      <style>{`body{cursor:${isDragging ? 'grabbing' : 'default'}}`}</style>
                  ...
                  <div className="task-row-bottom">
                    ...
                    {!isZero && (
                      <Tooltip text={task.locked ? 'MM 비율이 고정되어 있습니다.\n드래그·균등배분으로 변경되지 않습니다.\n클릭하여 해제' : 'MM 비율 고정\n다른 항목 조정 시 이 항목은 변경되지 않습니다'} dir="up" align="end">
                        <button className={`lock-btn${task.locked ? ' locked' : ''}`} onClick={() => toggleLock(task.id)}>
